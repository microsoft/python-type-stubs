...
def axis0_safe_slice(
    X: NDArray | ArrayLike, mask: NDArray, len_mask: int
) -> ndarray: ...
def _array_indexing(
...
def _determine_key_type(
    key: Union[ndarray, str, Tuple[int], int, List[str]], accept_slice: bool = True
) -> str: ...
def _safe_indexing(
    X: Union[Series, ndarray, csr_matrix, DataFrame, List[str]],
    indices: Union[ndarray, List[str], Tuple[int], int],
    *,
    axis=0
) -> Any: ...
def _get_column_indices(
    X: Union[DataFrame, ndarray], key: Union[int, List[str], Tuple[int]]
) -> List[int]: ...
def resample(
    *arrays,
    replace: bool = True,
...
def shuffle(
    *arrays, random_state: int | RandomState | None = None, n_samples: int | None = None
) -> Sequence: ...
...
def gen_even_slices(
    n: int, n_packs: int, *, n_samples: int | None = None
) -> Iterator[slice]: ...
...
def _print_elapsed_time(
    source: Optional[str], message: Optional[str] = None
) -> Iterator[None]: ...
...
def _approximate_mode(
    class_counts: ndarray, n_draws: int, rng: RandomState
) -> ndarray: ...
def check_matplotlib_support(caller_name: str) -> None: ...
def check_pandas_support(caller_name: str): ...
def all_estimators(
    type_filter: Literal["classifier", "regressor", "cluster", "transformer"]
    | Sequence[str]
    | None = None
) -> ArrayLike: ...
